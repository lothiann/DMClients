...
// ----------------------------------------------------------------------------
// Global SOCKS5 proxy for all HTTP requests (set by CSocks5Proxy).
// ----------------------------------------------------------------------------
static std::mutex g_Socks5ProxyMutex;
static char g_aSocks5ProxyAddr[256] = {0};
static bool g_Socks5ProxyActive = false;

void http_set_socks5_proxy(const char *pAddr)
{
        std::lock_guard<std::mutex> Lock(g_Socks5ProxyMutex);
        if(pAddr && pAddr[0])
        {
                str_copy(g_aSocks5ProxyAddr, pAddr, sizeof(g_aSocks5ProxyAddr));
                g_Socks5ProxyActive = true;
        }
        else
        {
                g_aSocks5ProxyAddr[0] = '\0';
                g_Socks5ProxyActive = false;
        }
}
...
#pragma GCC diagnostic pop
        ...
        // Route through the global SOCKS5 proxy if one is active (set by
        // CSocks5Proxy). CURLPROXY_SOCKS5_HOSTNAME makes curl resolve hostnames
        // remotely via the proxy (no local DNS leak).
        {
                std::lock_guard<std::mutex> Lock(g_Socks5ProxyMutex);
                if(g_Socks5ProxyActive && g_aSocks5ProxyAddr[0])
                {
                        curl_easy_setopt(pH, CURLOPT_PROXY, g_aSocks5ProxyAddr);
                        curl_easy_setopt(pH, CURLOPT_PROXYTYPE, (long)CURLPROXY_SOCKS5_HOSTNAME);
                }
        }
