...
int CControls::SnapInput(int *pData)
{
	...
	else
	{
		...
		// --- ЛОГИКА БОТА ---
		CClient *pRealClient = (CClient *)Client();
		if(pRealClient && pRealClient->m_pBotControl)
		{
			// Прыжок, хук, движение, стрельба (как было)
			if(pRealClient->m_pBotControl->IsBotActiveJump())
				m_aInputData[g_Config.m_ClDummy].m_Jump = pRealClient->m_pBotControl->GetBotJump() ? 1 : 0;
			if(pRealClient->m_pBotControl->IsBotActiveHook())
				m_aInputData[g_Config.m_ClDummy].m_Hook = pRealClient->m_pBotControl->GetBotHook() ? 1 : 0;
			if(pRealClient->m_pBotControl->IsBotActiveMove())
				m_aInputData[g_Config.m_ClDummy].m_Direction = pRealClient->m_pBotControl->GetBotMove();
			if(pRealClient->m_pBotControl->IsBotActiveFire() && pRealClient->m_pBotControl->GetBotFire())
			{
				m_aInputData[g_Config.m_ClDummy].m_Fire++;
				m_aInputData[g_Config.m_ClDummy].m_Fire &= 63;
			}

			// Приоритет: сначала абсолютный прицел (c_oaim)
			if(pRealClient->m_pBotControl->IsBotActiveOverrideAim())
			{
				m_aMousePos[g_Config.m_ClDummy].x = (float)pRealClient->m_pBotControl->m_OverrideTargetX;
				m_aMousePos[g_Config.m_ClDummy].y = (float)pRealClient->m_pBotControl->m_OverrideTargetY;
			}
			else if(pRealClient->m_pBotControl->IsBotActiveAim())
			{
				m_aMousePos[g_Config.m_ClDummy].x += (float)pRealClient->m_pBotControl->m_TargetX;
				m_aMousePos[g_Config.m_ClDummy].y += (float)pRealClient->m_pBotControl->m_TargetY;
			}

			if(pRealClient->m_pBotControl->IsBotActiveOverrideAim() || pRealClient->m_pBotControl->IsBotActiveAim())
			{
				ClampMousePos();
				m_aInputData[g_Config.m_ClDummy].m_TargetX = (int)m_aMousePos[g_Config.m_ClDummy].x;
				m_aInputData[g_Config.m_ClDummy].m_TargetY = (int)m_aMousePos[g_Config.m_ClDummy].y;
