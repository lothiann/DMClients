...
// ----------------------------------------------------------------------------
// SOCKS5 proxy hooks (optional). When installed, net_udp_send/recv route all
// UDP through a SOCKS5 UDP relay instead of going direct. The direct variants
// above (net_udp_send_direct / net_udp_recv_direct) are used by the proxy
// itself to talk to its relay socket without recursing.
// ----------------------------------------------------------------------------

static SOCKS5_UDP_SEND_HOOK g_Socks5SendHook = nullptr;
static SOCKS5_UDP_RECV_HOOK g_Socks5RecvHook = nullptr;

void net_set_socks5_hooks(SOCKS5_UDP_SEND_HOOK pSend, SOCKS5_UDP_RECV_HOOK pRecv)
{
        g_Socks5SendHook = pSend;
        g_Socks5RecvHook = pRecv;
}

int net_udp_send(NETSOCKET sock, const NETADDR *addr, const void *data, int size)
{
        if(g_Socks5SendHook)
                return g_Socks5SendHook(addr, data, size);
        return net_udp_send_direct(sock, addr, data, size);
}

int net_udp_recv(NETSOCKET sock, NETADDR *addr, unsigned char **data)
{
        if(g_Socks5RecvHook)
                return g_Socks5RecvHook(addr, data);
        return net_udp_recv_direct(sock, addr, data);
}
...
void net_set_socket_timeout(NETSOCKET sock, int timeout_ms)
{
        ...
        if(sock->ipv4sock >= 0)
        {
                setsockopt(sock->ipv4sock, SOL_SOCKET, SO_SNDTIMEO, &Tv, sizeof(Tv));
                setsockopt(sock->ipv4sock, SOL_SOCKET, SO_RCVTIMEO, &Tv, sizeof(Tv));
        }
        if(sock->ipv6sock >= 0)
        {
                setsockopt(sock->ipv6sock, SOL_SOCKET, SO_SNDTIMEO, &Tv, sizeof(Tv));
                setsockopt(sock->ipv6sock, SOL_SOCKET, SO_RCVTIMEO, &Tv, sizeof(Tv));
        }
#elif defined(CONF_FAMILY_WINDOWS)
        DWORD Timeout = (DWORD)timeout_ms;
        if(sock->ipv4sock >= 0)
        {
                setsockopt(sock->ipv4sock, SOL_SOCKET, SO_SNDTIMEO, (const char *)&Timeout, sizeof(Timeout));
                setsockopt(sock->ipv4sock, SOL_SOCKET, SO_RCVTIMEO, (const char *)&Timeout, sizeof(Timeout));
        }
        if(sock->ipv6sock >= 0)
        {
                setsockopt(sock->ipv6sock, SOL_SOCKET, SO_SNDTIMEO, (const char *)&Timeout, sizeof(Timeout));
                setsockopt(sock->ipv6sock, SOL_SOCKET, SO_RCVTIMEO, (const char *)&Timeout, sizeof(Timeout));
        }
#endif
}
